...
var fakeDb = {
  allDocs: function (options) {
    const filteredRows = _.filter(rows, row => {
      options.keys.indexOf(row.doc._id) > -1
    })
...
test('removeEntries should return a promise', t => {
  let rowIds = _.chain(rows).pluck('doc').pluck('_id').value()
  let res    = removeEntries(fakeDb, rowIds)
...
test('removeEntries calls db.allDocs and db.bulkDocs', t => {
  ...
  let rowIds = _.chain(rows).pluck('doc').pluck('_id').value()
  removeEntries(fakeDb, rowIds).then(() => {
    t.plan(2)
